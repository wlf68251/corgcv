...
def get_cameo_top2(code):
    """
    从 GDELT EventRootCode / EventCode 提取 CAMEO 顶层两位码。
...
def map_gdelt_relation_type(row):
    """
    cameotop 分支：
    GDELT EventRootCode / EventCode -> relation_type = 01~20。
    """
    ...
    if root in {f"{i:02d}" for i in range(1, 21)}:
        return root
...
def load_seed_edges():
    """
    读取 05 输出的 relation_edges_seed.csv。

    cameotop 分支要求：
        relation_type 已经是 01~20。
    """
    ...
    for _, row in seed_df.iterrows():
        ...
        relation_type = safe_str(row["relation_type"]).zfill(2)

        if relation_type not in {f"{i:02d}" for i in range(1, 21)}:
            continue

        if not subject_org_id or not object_org_id or not event_month:
            continue
